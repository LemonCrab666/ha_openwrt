...
class DataFetcher:
    ...
    async def _get_openwrt_status(self, sysauth):
        ...
        if resdata.get("wan_6"):
            self._data["openwrt_wan6_ip"] = resdata["wan_6"]["ip6addr"]
            self._data["openwrt_wan6_ip_attrs"] = resdata["wan_6"]
            try:
                self._data["openwrt_wan6_uptime"] = self.seconds_to_dhms(resdata["wan_6"]["uptime"]) 
            except Exception:
                self._data["openwrt_wan6_uptime"] = resdata["wan_6"]["uptime"]
        else:
                ...
                if interface["interface"].lower() == 'wan':
                    ...
                    if len(ipv4_addresses) > 0:
                        ...
                        try:
                            ...
                            self._data["openwrt_wan_uptime"] = interface['uptime']
                elif interface['interface'].lower() == 'wan_6':
